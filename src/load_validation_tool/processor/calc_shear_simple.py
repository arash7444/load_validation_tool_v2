...
from data_readers.read_LiDAR_data import (
    load_lidar_data,
    load_lidar_data_10min,  # lidar: csv → (avg, heights, std)
)
...
def _shear_from_profiles(
    wsp_profiles: pd.DataFrame, window: int = 6, name: str = "alpha"
):
    """
...
def plot_shear_series(
    ...
    ax = axes[1]
    ax.plot(
        alpha_roll_med.index,
        alpha_roll_med.values,
        "o-",
    ...
    ax = axes[2]
    ax.plot(
        alpha_roll_mean.index,
        alpha_roll_mean.values,
        "o-",
...
def calc_shear_metmast_nc(met_files: Sequence[Union[str, Path]], roll_window: int = 6):
    """
    ...
    wsp_profiles = mast_df.pivot_table(
        index=mast_df.index, columns="height", values="wind_speed"
    ).sort_index()
    ...
    return _shear_from_profiles(
        wsp_profiles, window=roll_window, name="alpha_mast"
    )  # now returns: alpha_mast, alpha_mast_err, alpha_mast_roll_med, alpha_mast_roll_mean
...
def calc_shear_lidar_csv(
    lidar_files: Sequence[Union[str, "Path"]],
